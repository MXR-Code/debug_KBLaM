...
from experiment_train_config import parser  # 导入训练配置解析器
...
def get_prefix_str(args):  # 生成实验前缀字符串
    ...
    prefix_string = f"stage1_lr_{lr}"  # 前缀字符串包含学习率
    ...
    return prefix_string  # 返回生成的前缀字符串
...
if __name__ == "__main__":
    ...
    if args.verbose:
        logger.setLevel(logging.DEBUG)  # 如果是详细模式，设置为DEBUG级别
    else:
        logger.setLevel(logging.INFO)  # 否则设置为INFO级别

    print(vars(args))  # 打印参数字典

    dataset_name = args.train_dataset  # 获取训练数据集名称
    seed = args.seed  # 获取随机种子
    N = args.num_train_sample  # 获取数据集大小
    ...
    use_cached_embd = args.use_cached_embed  # 是否使用缓存的嵌入
    ...
    sep_query_head = args.separate_query_head  # 是否分离查询头
    ...
    kb_size = kb_size if kb_size is not None else dynamic_kb_size  # 设置知识库大小

    gradient_accm_step = args.gradient_accumulation_step  # 获取梯度累积步数

    length_invariance = args.length_invariance  # 是否长短不变
    outlier_num = args.outlier_num  # 异常值数量
    multi_entities = args.multi_entity  # 多重实体数量
    ...
    hf_model_spec = args.hf_model_specification  # HuggingFace模型规格
    ...
    if Accelerator().is_main_process:  # 仅主进程初始化wandb
        wandb.init(  # 设置wandb项目
            project="kb-llm",
            # 跟踪超参数和运行元数据
            config={
                "learning_rate": args.lr,  # 学习率
                'sep_query_head': sep_query_head,  # 是否分离查询头
                'kb_size': kb_size,  # 知识库大小
                'length_invariance': length_invariance,  # 是否长短不变
                'dataset': dataset_name,  # 数据集名称
                'outlier_num': outlier_num,  # 异常值数量
                'multi_entities': multi_entities,  # 多重实体数量
                'use_extended_qa': use_extended_qa,  # 是否使用扩展QA
                'kb_token_layer_frequency': kb_token_layer_frequency,  # KB token层频率
                'gradient_accm_step': gradient_accm_step,  # 梯度累积步数
                "encoder_spec": encoder_spec,  # 编码器规格
    ...
    if args.log_to_file:  # 如果需要记录到文件
        formatter = logging.Formatter(LOGFORMAT)  # 设置日志格式化器
        f_handler = logging.FileHandler(model_save_dir / "log.txt")  # 创建文件处理器
        f_handler.setFormatter(formatter)  # 设置格式化器
        logger.addHandler(f_handler)  # 添加文件处理器到记录器

    logger.info(f"Running on {device}")  # 记录当前运行设备

    logger.info("🚨 Started training 🚨")  # 记录训练开始
    logger.info(f"💽 Saving to  {model_save_dir}💽")  # 记录模型保存目录
    if sep_query_head:
        os.environ["SEP_QUERY_HEAD"] = "TRUE"  # 设置环境变量以分离查询头
        logger.info("Having separate query head for KB!")  # 记录信息：采用分离查询头

    if length_invariance:
        os.environ["LENGTH_INVARIANCE"] = "TRUE"  # 设置环境变量以表示长度不变
        logger.info("Having separate query head for KB!")  # 记录信息：采用分离查询头

    os.environ["SCALE_FACTOR"] = ""  # 设置缩放因子环境变量

    if use_cached_embd:  # 如果使用缓存的嵌入
        # 我们从磁盘加载预计算版本，快速化处理
        logger.info(f"Using pre-computed {encoder_spec} embedding")  # 记录信息：使用预计算嵌入
        key_embds, value_embds = load_cached_embeddings(encoder_spec, dataset_dir, dataset_name,
                                                        key_embd_src)  # 加载缓存的嵌入

    prefix_string = get_prefix_str(args)  # 获取实验前缀字符串
    logger.info(f"Experiment prefix {get_prefix_str(args)}")  # 记录实验前缀

    if use_extended_qa:  # 如果使用扩展QA
        dataset = json.load(
            open(os.path.join(dataset_dir, f"{dataset_name}_augmented.json")))  # 加载扩展数据集
    else:
        dataset = json.load(open(os.path.join(dataset_dir, f"{dataset_name}.json")))  # 加载常规数据集

    training_set = dataset[:N]  # 划分出训练集

    # 设置LLM模型
    llm_model_spec = model_dir_to_resume if model_dir_to_resume else hf_model_spec  # 选择恢复模型或HuggingFace模型规格

    resumed_step = 0 if not model_dir_to_resume else int(model_dir_to_resume.split("_")[-1])  # 获取恢复的步骤

    if llm_model_spec is None:
        raise ValueError(
            "Either supply model_dir_to_resume or hf_model_spec")  # 必须提供模型目录或HuggingFace模型规格

    if hf_token is None and args.llm_type == "llama3":  # 如果使用Llama3模型且未提供令牌
        raise ValueError(
            "Please supply HuggingFace token(hf_token) when loading model Llama weights from HuggingFace")  # 抛出错误，需提供令牌
    ...
    if args.llm_type == "llama3":  # 如果选择Llama3
        model = KblamLlamaForCausalLM.from_pretrained(llm_model_spec,  # 从HuggingFace加载Llama模型
                                                      device_map=device,
        ...
        model = KBLaMPhi3ForCausalLM.from_pretrained(llm_model_spec,  # 从HuggingFace加载Phi3模型
                                                     device_map=device,
                                                     torch_dtype="auto",
                                                     trust_remote_code=True,
                                                     )
    ...
    logger.info(model.config)  # 记录模型配置
    ...
    # 设置编码器
    encoder = KBEncoder(encoder_name=encoder_spec,  # 创建知识库编码器
    ...
    kbretriever = KBRetriever(encoder,  # 创建KBRetriever对象
                              training_set,
                              key_embds=key_embds,  # 用加载的键嵌入
                              value_embds=value_embds,  # 用加载的值嵌入
                              )

    logger.info("Model ready 🚀")  # 记录模型准备就绪的信息

    # 开始训练
    llm_ckpt_name = f"{prefix_string}KeyFrom{key_embd_src}_{encoder_spec}_{dataset_name}_{llm_type}"  # 创建checkpoint名称
    ...
    logger.info(f"Number of trainable parameters: {get_parameter_count(encoder):,}")  # 记录可训练参数的数量
