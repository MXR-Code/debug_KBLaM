...
from model_kblam_gpt_session import GPT
...
def get_projector(projector_type: str, in_dim: int, out_dim: int, projector_kwargs: dict) -> nn.Module:
    ...
    if projector_type == "identity":
        ...
        mlp_depth, mlp_hidden_dim = (projector_kwargs["mlp_depth"],
...
class KBEncoder(nn.Module, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        if encoder_name in ["OAI", "BigOAI"]:
            ...
            if get_oai_embd_online:
                if big:
                    self.gs = GPT(model_name="text-embedding-3-large", endpoint_url=endpoint_url)
                else:
                    self.gs = GPT(model_name="ada-embeddings", endpoint_url=endpoint_url)

                self.base_model_encode = self.base_model_encode_1
            else:
                self.base_model_encode = None
            self.in_dim = 3072 if big else 1536
        else:
            self.base_model = SentenceTransformer(encoder_name)
            self.base_model_encode = self.base_model_encode_2
    ...
    def base_model_encode_1(self, s):
        out = self.gs.generate_embedding(s)
        out = torch.tensor(out).to(self.device)
        return out

    def base_model_encode_2(self, s):
        out = self.base_model.encode(s, convert_to_numpy=False)
        return out
