...
class LlamaAttention_KBLaM(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        assert (self.head_dim * self.num_head) == self.hidden_size

        self.score_shift = nn.Parameter(torch.zeros(self.num_head, 1) - 3)
        self.q_proj_new = nn.Linear(in_features=self.hidden_size,
                                    out_features=self.num_head * self.head_dim,
                                    bias=config.attention_bias)
    ...
    def tensor_parallel(self, hidden_states):
        # 预训练张量并行度 (Pretraining Tensor Parallelism)
        if self.config.pretraining_tp > 1:
            key_value_slicing = (self.num_key_value_heads * self.head_dim) // self.config.pretraining_tp
            split_size = (self.num_head * self.head_dim) // self.config.pretraining_tp
        ...
        return query_states, query_states_2, key_states, value_states
    ...
    def forward(self,
                ...
                kb_key_value: Optional[tuple] = None,
        ...
        query_states, query_states_2, key_states, value_states = self.tensor_parallel(hidden_states=hidden_states)

        query_states = query_states.view(batch_size, q_len, self.num_head, self.head_dim)
        query_states = query_states.transpose(dim0=1, dim1=2)

        query_states_2 = query_states_2.view(batch_size, q_len, self.num_head, self.head_dim)
        ...
        if kb_key_value is not None:
            if self.layer_idx % kb_layer_frequency == 0:
                kb_key, kb_value = kb_key_value  # (kb_len, head_dim * num_heads * num_adapters)

                # Should be something inside the kb config [what??]
                # kb_idx = self.layer_idx // kb_layer_frequency

                if len(kb_key.shape) == 2:  # Not batch dim
                    kb_len = kb_key.shape[0]
                    ...
                    shape = (kb_len, self.num_head, self.head_dim)
                    kb_key = kb_key.view(shape).transpose(0, 1)

                    shape = (kb_len, self.num_head, self.head_dim)
                    kb_value = kb_value.view(shape).transpose(0, 1)

                    shape = (batch_size, self.num_head, kb_len, self.head_dim)
                    kb_key = kb_key.unsqueeze(0).expand(shape)

                    shape = (batch_size, self.num_head, kb_len, self.head_dim)
                    kb_value = kb_value.unsqueeze(0).expand(shape)

                    if dynamic_sparsify:
                        kb_key, kb_value, attn_weights_2 = self.prune_key_value(query=query_states_2,
                                                                                  kb_keys=kb_key,
                                                                                  kb_values=kb_value,
                                                                                  topk_size=topk_size)
                    # Append the KB keys and values in the front, in front of padding
                    # batch_size, num_head, kb_len+q_len, head_dim
                    key_states = torch.concat([kb_key, key_states], dim=2)
                    value_states = torch.concat([kb_value, value_states], dim=2)

                elif len(kb_key.shape) == 3:  # Has a batch dim
                    kb_len = kb_key.shape[1]

                    shape = (batch_size, kb_len, 1 + self.config.num_hidden_layers // kb_layer_frequency, -1)
                    kb_key = kb_key.view(shape)
                    # kb_keys = kb_keys[:, :, kb_idx]

                    shape = (batch_size, kb_len, 1 + self.config.num_hidden_layers // kb_layer_frequency, -1,)
                    kb_value = kb_value.view(shape)
                    # kb_values = kb_values[:, :, kb_idx]

                    kb_key = kb_key.view(batch_size, kb_len, self.num_head, self.head_dim)
                    kb_key = kb_key.transpose(1, 2)

                    kb_value = kb_value.view(batch_size, kb_len, self.num_head, self.head_dim)
                    kb_value = kb_value.transpose(1, 2)
                    ...
                    if dynamic_sparsify:
                        kb_key, kb_value, attn_weights_2 = self.prune_key_value(query=query_states_2,
                                                                                  ...
                                                                                  topk_size=topk_size)
                    # Append the KB keys and values in the front, in front of padding
                    key_states = torch.concat(tensors=[kb_key, key_states], dim=2)
                    value_states = torch.concat(tensors=[kb_value, value_states], dim=2)

                # Modify the attention matrix: Appendx a (seq_len, kb_len) block to the left
                kb_len = kb_key.shape[2]
            ...
            if kb_key_value is not None:
                if self.layer_idx % kb_layer_frequency == 0:
                    ...
                    if attn_weights_2 is None:
                        attn_weights_2 = torch.matmul(query_states_2, kb_key.transpose(2, 3)) / math.sqrt(
                            self.head_dim)
        ...
        if attn_output.size() != (batch_size, self.num_head, q_len, self.head_dim):
            raise ValueError(f"`attn_output` should be of size {(batch_size, self.num_head, q_len, self.head_dim)},"
...
#                      rope_theta=10000
#                      )
# attention_module = LlamaAttention_KBLaM(config, layer_idx=0)
#
# # 简单输入数据
# batch_size = 1
# q_len = 4
